...
import { StudyType, TriagePriority, VisitType, StudyStatus } from './enums/index';
import type { RegisterVisitDto } from './dto/register-visit.dto';
import type { OrchestratorResultDto, AssignedStudyDto } from './dto/orchestrator-result.dto';
import { resolveStudyOrder, getStudyDependencies, PREPARATION_MAP } from './rules/dependency.rules';
...
export class OrchestatorService {
	...
	completeStudy(
		...
		if (!entry) throw new Error(`Estudio ${studyType} no encontrado en la visita ${visitId}.`);
		...
		// Remover de la cola
		const queue = this.queues.get(studyType)!;
		const idx = queue.findIndex((e) => e.visitId === visitId && e.studyType === studyType);
		...
		for (const dep of visit.entries) {
			...
			const allDepsComplete = dep.dependsOn.every((d) =>
				visit.entries.some((e) => e.studyType === d && e.status === StudyStatus.COMPLETED)
			);

			if (allDepsComplete) {
				dep.status = StudyStatus.QUEUED;
				unblocked.push(dep.studyType);
				this.logger.log(`Estudio ${dep.studyType} desbloqueado para visita ${visitId}`);
			}
		}
	...
	getQueueSummary(): Record<string, number> {
		...
		for (const [type, queue] of this.queues) {
			summary[type] = queue.filter((e) => e.status !== StudyStatus.COMPLETED).length;
		}
		return summary;
	}
	...
	private calculateTriage(dto: RegisterVisitDto): TriagePriority {
		...
		if (dto.visitType === VisitType.SCHEDULED) return TriagePriority.APPOINTMENT;
